...
To illustrate the Blind Synchronization Effect in a controlled setting, we construct a simple two-niche environment with deterministic regime switching.

\textbf{Setup.} Environment alternates between ``Regime A'' (niche 1 fitness = 1.1, niche 2 fitness = 0.9) and ``Regime B'' (niche 1 fitness = 0.9, niche 2 fitness = 1.1) every 50 timesteps. We run $n=10$ replicators for $T=500$ timesteps.

\textbf{Observations.}
\begin{enumerate}[leftmargin=2em,topsep=0pt,itemsep=2pt]
\item SI starts at 0 (uniform affinities) and rises as replicators specialize.
\item During regime transitions, SI temporarily dips as replicators re-adapt.
\item SI stabilizes at $\sim$0.75 after $\sim$100 timesteps.
\item SI-regime correlation: $r = 0.42$ ($p < 0.001$).
\end{enumerate}

This toy example demonstrates: (1) SI emerges from competition alone, (2) SI tracks environmental structure despite replicators never observing regimes, and (3) the phase transition timescale depends on fitness differential.
