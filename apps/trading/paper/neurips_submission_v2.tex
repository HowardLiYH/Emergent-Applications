...
\textbf{Note on terminology:} Our ``agents'' are \textit{not} LLM-based or neural network agents. They are simple \textbf{strategy instances}---each agent is merely an affinity vector $\mathbf{p}_i$ that evolves via multiplicative weight updates. There is no learning, reasoning, or planning. This simplicity is intentional: we show that even minimal agents exhibit emergent market-correlated behavior through competition alone.
...
\end{equation}
This exceeds SI-ADX correlation ($r = 0.13$), suggesting SI captures market extremity even more than trend strength. When markets are strongly overbought or oversold, agents specialize more intensely.
